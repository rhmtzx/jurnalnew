                    ...
                    <div class="col s12">
                        <div class="card">
                            <div class="card-content">
                                <h1 class="card-title">Data Pembimbing Dudi</h1>
                                <h6 class="card-subtitle">Seluruh Data Pembimbing Dudi Ada Disini!</h6>
                                <br>
                                <a href="/tambahdatapersyaratan" class="btn btn-success mb-10">Tambah Pembimbing Dudi +</a>
                                <br>
                                
                                <div class="table-responsive">
                                    <table id="pembimbingdudi" class="table text-center table-bordered dt-responsive nowrap"
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($data as $row)
                                                <tr>
                                                    ...
                                                    <td>{{ $row->namapemdudi }}</td>
                                                    <td>{{ $row->alamatpemdudi }}</td>
                                                    <td>{{ $row->notlppemdudi }}</td>
                                                    <td>{{ $row->id_siswa }}</td>
                                                    <td>{{ $row->id_dudi }}</td>
                                                    <td>{{ $row->created_at}}</td>
                                                    <td scope="row">
                                                        <a href="/tampilpembimbingdudi/{{ $row->id }}"
                                                            class="btn btn-warning"><i
                                                                class="fa-sharp fa-solid fa-pen-to-square"></i></a>
                                                        <a href="#" class="btn btn-danger deletepembimbingdudi"
                                                            data-id="{{ $row->id }}"
    ...
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js')}}"
        ...
        <script>
            $(document).ready(function() {
                $('#pembimbingdudi').DataTable();
            });
        </script>
</body>

<script>
    $('.deletepembimbingdudi').click(function() {
        ...
        .then((willDelete) => {
            if (willDelete) {
                window.location = "/deletepembimbingdudi/" + kategoriid + ""
